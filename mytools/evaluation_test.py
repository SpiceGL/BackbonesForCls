...
def main(): 
    ...
    input_names = ["x"]
    output_names = ["y"]
    # inp = torch.randn(1, 3, 256, 128)
    inp = np.full((1, 3, 160, 320), 1).astype(np.float) #(160,320) = (h,w)
    inp = torch.FloatTensor(inp)
    out = model(inp)
    print(out)

    # torch.onnx.export(model, inp, "./van230207.onnx", opset_version=10, verbose=True, input_names=input_names, output_names=output_names)
    # #---
