...
from myutils.train_utils import train, validation, validation2, print_info, netcfg2dict, pipelinecfg2dict, init_random_seed, set_random_seed, resume_model
from myutils.inference import init_model
...
def main():
    ...
    train_pipeline, val_pipeline, no_pipeline, _ = pipelinecfg2dict("mycfg/pipeline.py")
    ...
    # 制作数据集->数据增强&预处理
    threshold = 0.5  #开启no_pipeline的概率值
    train_dataset = Mydataset(train_datas, train_pipeline, no_pipeline, threshold)
    val_pipeline = copy.deepcopy(train_pipeline)
    val_dataset = Mydataset(val_datas, val_pipeline, no_pipeline, threshold)
    ...
    for epoch in range(runner.get('epoch'),runner.get('max_epochs')):
        ...
        validation2(model,runner2, data_cfg.get('test'), device, epoch, data_cfg.get('train').get('epoches'), meta)
