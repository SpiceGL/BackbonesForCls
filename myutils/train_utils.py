...
def pipelinecfg2dict(filename):
    ...
    return cfg_dict.get('train_pipeline'), cfg_dict.get('val_pipeline'), cfg_dict.get('no_pipeline'), cfg_dict.get("net_size")
...
def validation2(model, runner, cfg, device, epoch, epoches, meta):
    preds, targets = [], []
    model.eval()
    with torch.no_grad():
        with tqdm(total=len(runner.get('val_loader')), desc=f'Test : Epoch {epoch + 1}/{epoches}',
    ...
    if eval_results.get('accuracy_top-1') >= runner.get('best_val_acc'):
        ...
        runner['best_val_weight'] = os.path.join(meta['save_dir'], 'Val2_Epoch{:03}-Acc{:.3f}.pth'.format(epoch + 1,
                                                                                                         eval_results.get(
                                                                                                             'accuracy_top-1')))
        # meta['best_val_weight'] = runner['best_val_weight']
        save_checkpoint(model, runner.get('best_val_weight'), runner.get('optimizer'),
                        meta)
        # print(runner.get('best_val_weight'))
    #
    # if epoch > 0 and os.path.isfile(runner['last_weight']):
    #     os.remove(runner['last_weight'])
    # runner['last_weight'] = os.path.join(meta['save_dir'],'Last_Epoch{:03}.pth'.format(epoch+1))
    # meta['last_weight'] = runner['last_weight']
    # save_checkpoint(model,runner.get('last_weight').split(".")[0] + "_val2last.pth",runner.get('optimizer'),meta)
